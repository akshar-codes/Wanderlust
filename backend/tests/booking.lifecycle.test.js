...
describe("Booking Lifecycle Tests", () => {
  ...
  beforeEach(async () => {
    ...
    host = await User.create({ username: `host_${Date.now()}`, email: `host_${Date.now()}@test.com`, isActive: true });
    guest = await User.create({ username: `guest_${Date.now()}`, email: `guest_${Date.now()}@test.com`, isActive: true });
    otherUser = await User.create({ username: `other_${Date.now()}`, email: `other_${Date.now()}@test.com`, isActive: true });
    admin = await User.create({ username: `admin_${Date.now()}`, email: `admin_${Date.now()}@test.com`, isActive: true, role: "admin" });
  ...
  describe("Availability & validation", () => {
    it("rejects when listing not found", async () => {
      ...
      await expect(
        bookingService.createBooking(guest._id, { listingId: new mongoose.Types.ObjectId(), checkIn, checkOut, guestsCount: 2 })
    ...
    it("rejects inactive listing", async () => {
      ...
      const { checkIn, checkOut } = getDates(5, 3);
      await expect(
        bookingService.createBooking(guest._id, { listingId: listing._id, checkIn, checkOut, guestsCount: 2 })
    ...
    it("rejects owner booking own listing", async () => {
      const { checkIn, checkOut } = getDates(5, 3);
      await expect(
        bookingService.createBooking(host._id, { listingId: listing._id, checkIn, checkOut, guestsCount: 2 })
    ...
    it("rejects guest count exceeding maxGuests", async () => {
      const { checkIn, checkOut } = getDates(5, 3);
      await expect(
        bookingService.createBooking(guest._id, { listingId: listing._id, checkIn, checkOut, guestsCount: 5 })
  ...
  describe("Pricing correctness", () => {
    it("computes pricing accurately", async () => {
      const { checkIn, checkOut } = getDates(5, 3);
      const booking = await bookingService.createBooking(guest._id, { listingId: listing._id, checkIn, checkOut, guestsCount: 2 });
  ...
  describe("Creation", () => {
    it("returns populated booking, adds blockedDateId, increments counter", async () => {
      ...
      expect(updatedListing.availabilityCalendar[0]._id.toString()).toBe(booking.blockedDateId.toString());
  ...
  describe("Host Confirmation & Decline", () => {
    it("Host can confirm a pending booking", async () => {
      ...
      const confirmed = await bookingService.confirmBooking(booking._id, host._id);
    ...
    it("Host can decline a pending booking, frees up calendar", async () => {
      ...
      const declined = await bookingService.declineBooking(booking._id, host._id, "Not available");
  ...
  describe("Guest Cancellation", () => {
    it("Guest can cancel pending/confirmed booking, frees up calendar", async () => {
      ...
      const cancelled = await bookingService.cancelBooking(booking._id, guest._id, "Plans changed");
  ...
  describe("Completion", () => {
    it("Host can complete a confirmed booking after check-out", async () => {
      ...
      const completed = await bookingService.completeBooking(booking._id, host._id);
  ...
  describe("Admin override", () => {
    it("Admin can move booking to any status", async () => {
      ...
      const updated = await bookingService.adminUpdateBookingStatus(booking._id, "completed", "Admin override", admin._id);
