...
describe("Upload Security Tests", () => {
  ...
  describe("MIME type filter & Extension Validation", () => {
    it("Valid JPEG passes filter", async () => {
      ...
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/jpeg" });
    ...
    it("Valid PNG passes filter", async () => {
      ...
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/png" });
    ...
    it("image/gif rejected by filter", async () => {
      ...
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/gif" });
    ...
    it("application/pdf spoofed with .jpg extension rejected by filter if user sets bad mime", async () => {
      ...
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "application/pdf" });
      expect(res.statusCode).toBe(400);
    });
  });
  ...
  describe("Magic-byte / content spoofing (validateFileType)", () => {
    ...
    it("WebP passes filter and magic bytes", async () => {
      const webpMagic = Buffer.from([
        0x52, 0x49, 0x46, 0x46, 0x00, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42, 0x50,
      ]);
      const p = getFile("valid.webp", webpMagic);
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/webp" });
      expect(res.statusCode).toBe(200);
    });

    it("SVG rejected by MIME filter", async () => {
      const svgMagic = Buffer.from("<svg xmlns=");
      const p = getFile("bad.svg", svgMagic);
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/svg+xml" });
      expect(res.statusCode).toBe(400);
    });

    it("HTML file disguised as JPEG rejected (magic mismatch)", async () => {
      const htmlMagic = Buffer.from("<html><body></body></html>");
      const p = getFile("spoofed_html.jpg", htmlMagic);
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/jpeg" });
      expect(res.statusCode).toBe(400);
    ...
    it("Executable file disguised as PNG rejected", async () => {
      const exeMagic = Buffer.from([0x4d, 0x5a, 0x90, 0x00]); // MZ
      const p = getFile("spoofed_exe.png", exeMagic);
      const res = await request(app)
        .post("/upload/avatar")
        .attach("image", p, { contentType: "image/png" });
      expect(res.statusCode).toBe(400);
