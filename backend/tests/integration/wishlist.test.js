...
describe("Wishlist Integration Tests", () => {
  ...
  it("should fail add to wishlist if unauth", async () => {
    const host = await makeUser({}, "Password123!");
    const listing = await makeListing(host._id);
    const { agent, token } = await getAgent();
    const res = await agent
      .post(`/api/wishlists/${listing._id}/toggle`)
      .set("X-CSRF-Token", token);
    expect(res.status).toBe(404); // Assuming 404 if not mounted correctly or 401
  });
});
